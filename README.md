...
- **Numerical comparisons:** `greater` (>), `less` (<), `num_equal` (==) evaluated at query time on `Num` terms — and query-time ONLY: a comparison whose operands could be numbers is refused at assertion ingress, in a ground fact and in every rule position alike, because a query computes it while a rule would look it up in a store that holds none. A comparison between non-numeric terms (`greater(Alis, Bob)`, "taller than") is an ordinary relational fact and asserts normally
