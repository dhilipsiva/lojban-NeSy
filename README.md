...
> **Numbers are values, not entities.** A number never enters the quantifier domain, so a universal restricted to a number-bearing predicate is **vacuously true**: with `big(5).` asserted, `sum(every big, 2, 3).` is `TRUE` — and so is the arithmetically false `sum(every big, 2, 2).`, because `5` is never enumerated. Numbers participate in arithmetic and comparison, not in entity quantification or counting. Nibli announces this rather than answering silently: such a query emits a `[Domain]` line listing the skipped values, and a `numeric_domain_gap` step in the proof trace. The warning is checked, not guessed — each candidate value is confirmed by evaluating the restrictor at it, so a number the restrictor does not actually hold of is never reported. Quantify over entities and relate them to numbers in the body.
