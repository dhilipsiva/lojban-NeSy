...
1. **Locate & plan.** Find the exact item in `book/TODO.md`. Read the `file:line` locations it cites plus
   ...
   [`book/AGENTS.md`](../../../book/AGENTS.md); never invent code or engine output. State a one-paragraph
...
2. **Implement.** Follow the book house style in [`book/AGENTS.md`](../../../book/AGENTS.md): closed em-dashes
...
5. **Sync docs.** Per the book pre-commit checklist: update `book/AGENTS.md` / `book/README.md` if Lojban
...
6. **Commit & push (book repo).**
   ...
   - End the message with the `Co-Authored-By: Codex … <noreply@anthropic.com>` trailer the harness
